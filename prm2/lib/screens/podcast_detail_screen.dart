...
class _PodcastDetailScreenState extends State<PodcastDetailScreen> {
  ...
  Future<void> _loadUserProfile() async {
    try {
      ...
      if (result.isSuccess && result.data != null) {
        ...
        print('🔍 DEBUG: Loaded user profile - ID: "${result.data!.id}", UserId: "${result.data!.userId}", FullName: "${result.data!.fullName}"');
  ...
  Future<void> _loadPodcast() async {
    ...
    if (mounted) {
      if (result.isSuccess && result.data != null) {
        ...
        bool isLiked = false;
        final isOwnPodcast = _currentUserId != null && 
                             _currentUserId != 'FAILED_TO_LOAD' && 
                             _currentUserId == result.data!.createdBy;
        
        print('🔍 DEBUG: Podcast loaded - User ID: "$_currentUserId", Created By: "${result.data!.createdBy}"');
        print('🔍 DEBUG: User ID type: ${_currentUserId.runtimeType}, Created By type: ${result.data!.createdBy.runtimeType}');
        print('🔍 DEBUG: IDs equal: ${_currentUserId == result.data!.createdBy}');
        print('🔍 DEBUG: isCreatorView: ${widget.isCreatorView}, isOwnPodcast: $isOwnPodcast');
        
        if (!widget.isCreatorView && !isOwnPodcast && _currentUserId != null && _currentUserId != 'FAILED_TO_LOAD') {
          print('🔍 DEBUG: Checking like status...');
          isLiked = await ApiService.checkPodcastLiked(widget.podcastId);
        } else {
          print('🔍 DEBUG: Skipping like check - Creator view: ${widget.isCreatorView}, Own podcast: $isOwnPodcast, User ID loaded: ${_currentUserId != null}, User ID valid: ${_currentUserId != 'FAILED_TO_LOAD'}');
        }
        ...
        // Track view once (only for user view, not creator, not own podcast, and not from mini player)
        if (!widget.isCreatorView && !isOwnPodcast && !widget.fromMiniPlayer && !_viewTracked && _currentUserId != null && _currentUserId != 'FAILED_TO_LOAD') {
          print('🔍 DEBUG: Tracking view for podcast ${widget.podcastId}');
          _viewTracked = true;
          ApiService.incrementPodcastView(widget.podcastId);
        } else {
          print('🔍 DEBUG: Skipping view tracking - Creator view: ${widget.isCreatorView}, Own podcast: $isOwnPodcast, From mini player: ${widget.fromMiniPlayer}, Already tracked: $_viewTracked, User ID loaded: ${_currentUserId != null}, User ID valid: ${_currentUserId != 'FAILED_TO_LOAD'}');
        }
  ...
  Future<void> _toggleLike() async {
    ...
    if (widget.isCreatorView || isOwnPodcast) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bạn không thể thích podcast của chính mình')),
      );
      return;
    }
